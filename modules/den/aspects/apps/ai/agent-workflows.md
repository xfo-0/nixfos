...
- For concurrent or long-running work, isolate into a jj workspace via `jjws add <name>` (own working-copy `@` under `~/ws/<repo>/<name>`); run it from the main checkout, clean up with `jjws rm <name>` after merge.
